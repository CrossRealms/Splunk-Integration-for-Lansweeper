...
class Lansweeper:
    ...
    def get_refresh_token(self):
        ...
        try:
            response = requests.post(url=self.auth_url, data=data, proxies=self.proxy_settings)
            ...
            self.logger.error('Error while refreshing the access token, error={}'.format(exception))
            sys.exit(1)
    ...
    def get_site_id(self, site_name):
        ...
        try:
            ...
            if status_code != 200:
                self.logger.warning('Error while fetching the site id for site={} status code= {}'.format(site_name, status_code))
                return status_code, response
        except Exception as exception:
            self.logger.exception('Error while fetching the site id for site={}, error={}'.format(site_name, exception))
            sys.exit(1)
        ...
        try:
            ...
            for profile in response_json['data']['me']['profiles']:
                if profile['site']['name'] in site_name:
                    sites.append({profile['site']['name']: profile['site']['id']})
            return 200, sites
        except Exception as exception:
            self.logger.exception('Error while parsing the response for the site id for site={}, error={}'.format(site_name, exception))
            sys.exit(1)
    ...
    def is_token_expired(self, status_code, response):
        ...
        try:
            ...
            if status_code == 400 and response.get('errors', [])[0].get('extensions', {}).get('code') == 'UNAUTHENTICATED':
                self.logger.info('Access token is expired. Calling the refresh token API')
                status, response = self.get_refresh_token()
                
                if status != 200:
                    self.logger.error('Error while refreshing the access token, status code={} response={}'.format(status, response))
                    return False
            ...
            self.logger.exception('Error while checking if the access token is expired, error={}'.format(exception))
        return False
    ...
    def get_asset_info(self, site_id, cursor, page):
        ...
        return True, cursor, asset_data in case of successful collection of the assets, and Flase, status_code and response otherwise
        """
        headers = {'Content-Type': 'application/json', 'Authorization': 'Bearer ' + self.access_token}
